...
-- 奋音
sgs.ai_skill_invoke.fenyin = sgs.ai_skill_invoke.jizhi
...
sgs.ai_skill_invoke.biluan = function(self, data)
	...
	for _, p in sgs.qlist(self.room:getAlivePlayers()) do
		...
		if self:isEnemy(p) and p:inMyAttackRange(self.player) then
			canAttackme = canAttackme + 1
		end
	end
	return #kingdoms - canAttackme > 3
end

-- 礼下
sgs.ai_skill_invoke.lixia = sgs.ai_skill_invoke.jizhi
