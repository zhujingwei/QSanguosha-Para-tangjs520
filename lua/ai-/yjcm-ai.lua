...
function sgs.ai_cardneed.pojun(to, card, self)
...
-- 新·破军
table.insert(sgs.ai_global_flags, "newpojunsource")
sgs.ai_skill_invoke.newpojun = function(self, data)
	local target = data:toPlayer()
	if not self:isFriend(target) then
		self.newpojunsource = target
		return true
	end
	return false
end
...
sgs.ai_skill_choice.newpojun_num = function(self, choices)
	return "" .. math.min(self.newpojunsource:getHp(), self.newpojunsource:getCards("he"):length())--self.player:getLostHp()
end
...
sgs.ai_cardneed.newpojun = sgs.ai_cardneed.pojun
