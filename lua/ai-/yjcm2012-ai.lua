...
sgs.ai_card_intention.AnxuCard = function(self, card, from, to)
	local more, less
...
sgs.ai_skill_playerchosen.zhuiyi = function(self, targets)
	...
	if lord and self:isFriend(lord) and self:isWeak(lord) then return lord end
...
sgs.ai_skill_cardask["@jiefan-discard"] = function(self, data)
	...
	for _, p in sgs.qlist(self.room:getAllPlayers()) do
		if self:isEnemy(p) and not self.player:inMyAttackRange(p, range_fix) then count = count + 1 end
	end
...
sgs.ai_skill_invoke.zishou = function(self, data)
	...
	return 2 + self.player:getLostHp() - can_save_card_num + peach_num  <= chance_value
end
...
-- 新·自守
sgs.ai_skill_invoke.newzishou = function(self, data)
	if self:needBear() then return true end
	if self.player:isSkipped(sgs.Player_Play) then return true end
	return false
end
