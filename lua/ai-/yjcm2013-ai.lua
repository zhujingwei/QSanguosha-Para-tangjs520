...
-- 精策
sgs.ai_skill_invoke.jingce = sgs.ai_skill_invoke.jizhi
...
-- 纵适
sgs.ai_skill_invoke.zongshih = sgs.ai_skill_invoke.jizhi
...
sgs.ai_skill_use_func.FenchengCard = function(card, use, self)
	...
	for i, p in sgs.qlist(self.room:getOtherPlayers(self.player)) do
		if self:damageIsEffective(p, sgs.DamageStruct_Fire, lastPlayer) then
...
sgs.ai_skill_discard.fencheng = function(self, discard_num, min_num, optional, include_equip)
	...
	elseif peaches == 2 and self.player:getHp() > 1 and length == min_num then
		for _, friend in ipairs(self.friends_noself) do
			if friend:getHp() <= 2 and self:damageIsEffective(friend, sgs.DamageStruct_Fire, self.player) then return {} end
		end
	end
