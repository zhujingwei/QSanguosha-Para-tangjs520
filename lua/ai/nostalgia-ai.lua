...
sgs.ai_cardneed.nostieji = sgs.ai_cardneed.tieji

-- 旧·集智
sgs.ai_skill_invoke.nosjizhi = sgs.ai_skill_invoke.biyue
...
sgs.ai_cardneed.nosleiji = function(to, card)
	return card:getSuit() == sgs.Card_Spade or card:isKindOf("Jink")
end
...
sgs.ai_cardneed.nosfuhun = sgs.ai_cardneed.fuhun
...
-- 旧·秘计
sgs.ai_skill_invoke.nosmiji = function(self, data)
	...
	for _,p in sgs.qlist(self.room:getAlivePlayers()) do
		...
		if self:isEnemy(p) and self:needKongcheng(p, true) then
			return true
		end
	end
end
...
sgs.ai_card_intention.ExtraCollateralCard = 10

sgs.ai_cardneed.nosmieji = sgs.ai_cardneed.mieji
