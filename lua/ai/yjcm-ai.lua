...
sgs.ai_skill_playerchosen.mingce_slash = function(self, targets)
...
sgs.ai_cardneed.mingce = function(to, card)
	return card:isKindOf("EquipCard") or card:isKindOf("Slash")
end
...
sgs.ai_cardneed.pojun = sgs.ai_cardneed.lieren
...
sgs.ai_cardneed.jujian = function(to, card)
...
sgs.ai_skill_invoke.quanji = function(self, data)
	...
	local juece_effect = (current and current:isAlive() and current:getPhase() ~= sgs.Player_NotActive and self:isEnemy(current) and current:hasSkill("nosjuece"))
...
sgs.ai_skill_invoke.zhiman = function(self, data)
	...
	if self:isFriend(target) then
		if self:needToLoseHp(target, self.player, nil, true) and target:getJudgingArea():isEmpty() and target:getEquips():length() == 0 then
			return true
		end
...
-- 新·破军
sgs.ai_skill_invoke.newpojun = function(self, data)
	local target = data:toPlayer()
	if not self:isFriend(target) then
		self.newpojun_source = target
		return true
	end
	return false
end

sgs.ai_skill_choice.newpojun_num = function(self, choices)
	return "" .. math.min(self.newpojun_source:getHp(), self.newpojun_source:getCards("he"):length())--self.player:getLostHp()
end

sgs.ai_skill_cardchosen.newpojun_dis = function(self, who, flags)
	local cards = sgs.QList2Table(who:getEquips())
	local handcards = sgs.QList2Table(who:getHandcards())
	if #handcards < 3 or handcards[1]:hasFlag("visible") then table.insert(cards,handcards[1]) end
...
sgs.ai_choicemade_filter.cardChosen.newpojun_dis = sgs.ai_choicemade_filter.cardChosen.dismantlement

sgs.ai_cardneed.newpojun = sgs.ai_cardneed.lieren
-- 节钺
