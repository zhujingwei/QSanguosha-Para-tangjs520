...
void Card::onUse(Room *room, const CardUseStruct &use) const{
    ...
    if (card_use.card->getTypeId() != TypeSkill) {
        ...
        if (card_use.to.size() == 1 && card_use.to.first())
...
void Card::use(Room *room, ServerPlayer *source, QList<ServerPlayer *> &targets) const{
    ...
    if (isKindOf("TrickCard")) {
        ...
        playTrickCardEmotion(room, player);
    }
...
void Card::playTrickCardEmotion(Room *room, ServerPlayer *player) const
{
    ...
    //在此不用考虑延时类锦囊，因为这类锦囊是在其判定生效后才播放特效动画，
    //并且由于DelayedTrick类重写了use虚函数，所以也不会调用到此函数
    if (isKindOf("Duel") || isKindOf("Nullification")
        || isKindOf("IronChain")) {
            return;
    }

    QString trickEmotion = QString("trick/%1").arg(objectName());
    room->setEmotion(player, trickEmotion);
}
