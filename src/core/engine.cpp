...
QReadWriteLock ReadWriteLockHelper::m_readWriteLock;
...
Engine::Engine()
{
    ...
    ReadLock readLockObj;
    QList<const Skill *> skillValues = skills.values();
    foreach (const Skill *skill, skillValues) {
...
void Engine::addHeroSkinTranslationEntry(const char *key, const char *value)
{
    heroSkinTranslations.insert(key, QString::fromUtf8(value));
}
...
void Engine::addPackage(Package *package) {
    ...
    if (!all_cards.isEmpty()) {
        card_packages.insert(package->objectName(), all_cards.count());
    }
    ...
    foreach (General *general, all_generals) {
        ...
        const QSet<QString> &extraSkills = general->getExtraSkillSet();
        foreach (const QString &skill_name, extraSkills) {
            if (skill_name.startsWith("#")) continue;
            QList<const Skill *> relatedSkills = getRelatedSkills(skill_name);
            foreach (const Skill *related, relatedSkills) {
                if (NULL != related) {
...
QString Engine::heroSkinTranslate(const QString &to_translate) const
{
    QStringList list = to_translate.split("\\");
    QString res;
    foreach (const QString &str, list) {
        res.append(heroSkinTranslations.value(str, str));
    }
    return res;
}
...
QList<const Skill *> Engine::getRelatedSkills(const QString &skill_name) const{
    ...
    QStringList relatedSkills = related_skills.values(skill_name);
    foreach (const QString &name, relatedSkills)
...
int Engine::getGeneralCount(bool include_banned/* = false*/, const QString &kingdom/* = QString()*/) const
{
    int total = 0;
...
QString Engine::getCurrentCardUsePattern() {
    RoomState *roomState = currentRoomState();
    if (NULL != roomState) {
        return roomState->getCurrentCardUsePattern();
    }
    else {
        return QString();
    }
...
CardUseStruct::CardUseReason Engine::getCurrentCardUseReason() {
    RoomState *roomState = currentRoomState();
    if (NULL != roomState) {
        return roomState->getCurrentCardUseReason();
    }
    else {
...
QStringList Engine::getSetupString() const{
    ...
    QString server_name = Settings::toBase64(Config.ServerName);
...
int Engine::getCardCount(bool include_banned) const
{
    int result = cards.length();

    if (!include_banned) {
        QStringList cardPackageKeys = card_packages.keys();
        foreach (const QString &card_package_name, cardPackageKeys) {
            if (ban_package.contains(card_package_name)) {
                result -= card_packages.value(card_package_name);
            }
        }
    }
...
QStringList Engine::getLords(bool contain_banned) const{
    ...
    // add intrinsic lord
    foreach (const QString &lord, lord_list) {
...
QStringList Engine::getRandomLords() const{
    ...
    QStringList generalNames = generals.keys();
    foreach (const QString &nonlord, generalNames) {
...
QStringList Engine::getLimitedGeneralNames(const QString &kingdom/* = QString()*/) const{
...
QStringList Engine::getRandomGenerals(int count, const QSet<QString> &ban_set/* = QSet<QString>()*/, const QString &kingdom/* = QString()*/) const{
...
void Engine::playSystemAudioEffect(const QString &name, bool continuePlayWhenPlaying/* = false*/) const {
    playAudioEffect(QString("audio/system/%1.ogg").arg(name), continuePlayWhenPlaying);
}

void Engine::playAudioEffect(const QString &filename, bool continuePlayWhenPlaying/* = false*/) const {
#ifdef AUDIO_SUPPORT
    ...
    Audio::play(filename, continuePlayWhenPlaying);
#endif
}
...
void Engine::blockAllRoomSignals(bool block)
{
    m_mutex.lock();
    foreach (QObject *const &roomObject, m_rooms) {
        Room *room = qobject_cast<Room *>(roomObject);
        if (room) {
            room->blockAllThreadSignals(block);
        }
        else {
            roomObject->blockSignals(block);
        }
    }
    m_mutex.unlock();

    QThread::currentThread()->blockSignals(block);
}

const QStringList &Engine::getRobotNames() const
{
    static QStringList robot_names;
    if (robot_names.isEmpty()) {
        robot_names = GetConfigFromLuaState(lua, "robot_names").toStringList();
    }

    return robot_names;
}

const QString &Engine::getRandomRobotName() const
{
    const QStringList &robotNames = getRobotNames();
    if (robotNames.isEmpty()) {
        static QString nullName = "";
        return nullName;
    }
    return robotNames.at(qrand() % robotNames.size());
}

void Engine::clearRooms()
{
    m_mutex.lock();
    m_rooms.clear();
    m_mutex.unlock();
...
void Engine::playHulaoPassBGM()
{
    const QString hulaoPassBGMPath = "audio/system/hulao-pass.ogg";
    if (QFile::exists(hulaoPassBGMPath)) {
        Audio::setCustomBackgroundMusicFileName(hulaoPassBGMPath);
