...
void GeneralOverview::addLines(const Skill *skill) {
    ...
    bool useHeroSkinSkillAudios = true;
    QStringList audioSources = G_ROOM_SKIN.getHeroSkinSKillAudios(getCurrentSelectedGeneralName(),
        skillName);
...
void GeneralOverview::on_tableWidget_itemSelectionChanged() {
    const General *general = getCurrentSelectedGeneral();
    const QString general_name = general->objectName();

    ui->generalPhoto->setPixmap(G_ROOM_SKIN.getCardMainPixmap(general_name));
    ui->changeHeroSkinButton->setVisible(HeroSkinContainer::hasSkin(general_name));
    ...
    foreach (const Skill *skill, skills) {
        skillNames.append(skill->objectName());
        addLines(skill);
    }
    QString heroSkinName = getCurrentSelectedGeneralHeroSkinName();
    if (!heroSkinName.isEmpty()) {
        SkillToAudioFiles skillToAudioFiles = G_ROOM_SKIN.getSkillToAudioFiles(heroSkinName);
        if (skillToAudioFiles.isEmpty()) {
            QStringList generalNameAndSkinIndex = heroSkinName.split("_");
            QString generalName = generalNameAndSkinIndex.at(0);
            int generalSkinIndex = generalNameAndSkinIndex.at(1).toInt();
            const General *general = getCurrentSelectedGeneral();
            ...
            if (!matchGeneralFullName.isEmpty()) {
                skillToAudioFiles = G_ROOM_SKIN.getSkillToAudioFiles(matchGeneralFullName);
            }
        }

        QStringList skillNameList = skillToAudioFiles.keys();
        foreach (const QString &skillName, skillNameList) {
            if (!skillNames.contains(skillName)) {
                addLinesHelper(skillName, skillToAudioFiles.value(skillName), true);
            }
        }
    }
    ...
    QString last_word;
    if (!heroSkinName.isEmpty()) {
        last_word = Sanguosha->heroSkinTranslate("~" + general_name + "-" + heroSkinName);
    ...
    ui->skillTextEdit->setText(general->getSkillDescription(true));

    updateChangeGeneralButtonState();
}
...
void GeneralOverview::askChangeSkin() {
    ...
    int nextSkinIndex = HeroSkinContainer::getNextSkinIndex(general_name, n);
    if (nextSkinIndex == n) {
...
void GeneralOverview::showEvent(QShowEvent *event)
{
...
QString GeneralOverview::getCurrentSelectedGeneralName() const
{
    int row = ui->tableWidget->currentRow();
    return ui->tableWidget->item(row, 0)->data(Qt::UserRole).toString();
}

const General *GeneralOverview::getCurrentSelectedGeneral() const
{
    return Sanguosha->getGeneral(getCurrentSelectedGeneralName());
}

void GeneralOverview::updateChangeGeneralButtonState()
{
    const General *general = getCurrentSelectedGeneral();
    const QString general_name = general->objectName();
...
QString GeneralOverview::getCurrentSelectedGeneralHeroSkinName() const
{
    QString generalName = getCurrentSelectedGeneralName();
    int generalSkinIndex = Config.value(QString("HeroSkin/%1").arg(generalName), 0).toInt();
    if (0 != generalSkinIndex) {
        return QString("%1_%2").arg(generalName).arg(generalSkinIndex);
    }
    else {
        return QString();
    }
...
void GeneralOverview::addLinesHelper(const QString &skillName, const QStringList &aduioSources,
    bool useHeroSkinSkillAudios/* = false*/)
{
    QString zh_CNSkillName = Sanguosha->translate(skillName);

    if (aduioSources.isEmpty()) {
        QCommandLinkButton *button = new QCommandLinkButton(zh_CNSkillName);
        button->setEnabled(false);
        button_layout->addWidget(button);
    }
    else {
        QRegExp rx(".+/(\\w+\\d?).ogg");
        for (int i = 0; i < aduioSources.length(); ++i) {
            QString audioSource = aduioSources[i];
            if (!rx.exactMatch(audioSource)) {
                continue;
            }

            QString button_text = zh_CNSkillName;
            if (aduioSources.length() != 1) {
                button_text.append(QString(" (%1)").arg(i + 1));
            }

            QCommandLinkButton *button = new QCommandLinkButton(button_text);
            button->setObjectName(audioSource);
            button_layout->addWidget(button);

            QString skill_line;
            QString filename = rx.capturedTexts().at(1);
            if (useHeroSkinSkillAudios) {
                QString heroSkinName = getCurrentSelectedGeneralHeroSkinName();
                if (!heroSkinName.isEmpty()) {
                    skill_line = Sanguosha->heroSkinTranslate("$" + filename + "-" + heroSkinName);
                    if (skill_line.startsWith("$")) {
                        skill_line = Sanguosha->heroSkinTranslate("$" + filename);
                    }
                }
            }
