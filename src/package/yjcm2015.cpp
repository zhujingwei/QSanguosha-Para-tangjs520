...
public:
    ...
    bool trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const
    {
        if (triggerEvent == PreDamageDone) {
            ...
            if (damage.card != NULL && damage.card->isKindOf("Duel") && damage.card->getSkillName() == objectName())
                damage.to->setFlags("zhanjuedraw");
...
//             }
            ...
            CardUseStruct use = data.value<CardUseStruct>();
            if (use.card != NULL && use.card->isKindOf("Duel") && use.card->getSkillName() == objectName())
                foreach (ServerPlayer *p, room->getAlivePlayers()) {
                    if (p == use.from) {
                        p->drawCards(1);
                        room->addPlayerMark(p, "zhanjuedraw");
                    }
                    if (p->hasFlag("zhanjuedraw")) {
                        p->setFlags("-zhanjuedraw");
                        p->drawCards(1);
                        room->addPlayerMark(p, "zhanjuedraw");
                    }
            }
...
public:
    ...
    bool isEnabledAtResponse(const Player *player, const QString &pattern) const
    {
        ...
        if (pattern == "slash")
            return player->getMark("Huomo_Slash") == 0;
        if (pattern == "jink")
            return player->getMark("Huomo_Jink") == 0;
        if (pattern == "peach" && player->getMark("Global_PreventPeach") == 0)
            return player->getMark("Huomo_Peach") == 0;
        if (pattern.contains("analeptic"))
            return player->getMark("Huomo_Peach") == 0 || player->getMark("Huomo_Analeptic") == 0;
        return false;
    }
    ...
    const Card *viewAs(const Card *originalCard) const
    {
        if (Sanguosha->getCurrentCardUseReason() == CardUseStruct::CARD_USE_REASON_RESPONSE
            || Sanguosha->getCurrentCardUseReason() == CardUseStruct::CARD_USE_REASON_RESPONSE_USE) {
                HuomoCard *card = new HuomoCard;
                card->setUserString(Sanguosha->getCurrentCardUsePattern());
                card->addSubcard(originalCard);
                return card;
        }

        const Card *c = Self->tag.value("huomo").value<const Card *>();
        if (c) {
            HuomoCard *card = new HuomoCard;
...
//                 if (resp.m_isUse)
            ...
            if (card != NULL && (card->isKindOf("BasicCard"))) {
                QString class_name = card->getClassName();
                if (card->isKindOf("Slash"))
                    class_name = "Slash";
                room->setPlayerMark(player, "Huomo_" + class_name, 1);
                QStringList huomoList = player->tag.value("huomoClassName").toStringList();
                huomoList << class_name;
                player->tag["huomoClassName"] = huomoList;
            }
        }     
