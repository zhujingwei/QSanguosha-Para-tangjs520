            ...
            if (effect.jink_num == 1) {
                ...
                for (int i = effect.jink_num; i > 0; --i) {
                    ...
                    if (!room->isJinkEffected(effect.to, asked_jink)) {
                        ...
                        //修复“连续两次八卦判定，第一次判定成功，若第二次也判定成功，
                        //会在播放八卦动画后，又继续播放闪动画”的问题
...
QString GameRule::getWinner(ServerPlayer *victim) const{
    ...
    if (room->getMode() == "06_3v3") {
        ...
        //修复“其他玩家均全部阵亡，本方还未曾亮过将，系统一直询问本方是否亮将，
        //直到本方主动亮将后判本方赢，否则无限询问下去”的问题
...
//当一名角色明置武将牌确定势力时，该势力的角色超过了总游戏人数的一半，则他视为野心家
//是否成为野心家按游戏总人数计算，与其他角色是否死亡无关
...
void GameRule::forcePlayerShowGenerals(ServerPlayer *player) const
{
    ...
    if (player->getGeneralName() == "anjiang") {
        ...
        //增加野心家势力（野心家与野心家之间也是不同势力，所以他们的国名分别是ye0,ye1,ye2...等）
...
bool BasaraMode::trigger(TriggerEvent triggerEvent, Room *room, ServerPlayer *player, QVariant &data) const{
    ...
    case BuryVictim: {
            ...
            //国战奖惩方式：1、已经确定势力的角色杀死相同势力的角色须弃置所有手牌和装备区的牌；
            //2、已经确定势力的角色杀死不同势力的角色，从牌堆摸取等同于该势力人数（包括刚刚杀死的角色）张牌。
            //注：若被杀死的角色还没有明置武将牌（即没确定势力），则须明置验明势力。
            //没有势力的角色（即武将牌没有明置的角色）杀死其他角色没有奖惩。
            if (Config.EnableHegemony) {
                ServerPlayer *killer = death.damage ? death.damage->from : NULL;
                //killer->getKingdom() != "god"即表示killer已经明置了武将牌，确定了势力
