...
Room::Room(QObject *parent, const QString &mode)
    : QThread(parent), mode(mode), current(NULL), pile1(Sanguosha->getRandomCards()),
      ...
      m_surrenderRequestReceived(false), _m_roomState(false), gameStartCommandNotified(false),
      startInXsCommandNotified(false)
{
...
void Room::initCallbacks() {
    ...
    m_callbacks[S_COMMAND_KICK_PLAYER] = &Room::kickPlayerCommand;
}
...
void Room::killPlayer(ServerPlayer *victim, DamageStruct *reason) {
    ...
    if (Config.EnableHegemony) {
        QString kingdom = victim->getKingdom();
    ...
    if (!victim->isAlive() && Config.EnableAI) {
        ...
        if (expose_roles) {
            foreach (ServerPlayer *player, m_alivePlayers) {
                if (Config.EnableHegemony) {
                    thread->getGameRule()->forcePlayerShowGenerals(player);
                } else
...
void Room::judge(JudgeStruct &judge_struct) {
    ...
    JudgeStruct *judge = &judge_struct;
    QVariant data = QVariant::fromValue(judge);
    thread->trigger(StartJudge, this, judge->who, data);
    ...
    thread->trigger(FinishRetrial, this, judge->who, data);
    thread->trigger(FinishJudge, this, judge->who, data);
}
...
void Room::gameOver(const QString &winner) {
    ...
    foreach (ServerPlayer *player, m_players) {
        ...
        if (player->getHandcardNum() > 0) {
            ...
            QString handcard = Settings::toBase64(handcards.join(", "));
...
void Room::slashResult(const SlashEffectStruct &effect, const Card *jink) {
    ...
    if (jink == NULL) {
        ...
        //所以此处不再重复处理
        if (effect.to->isAlive() && !effect.to->hasSkills("leiji|nosleiji")) {
...
void Room::detachSkillFromPlayer(ServerPlayer *player, const QString &skill_name, bool is_equip, bool acquire_only) {
    ...
    if (skill && skill->isVisible()) {
        ...
        foreach (const Skill *skill, relatedSkills) {
            if (NULL != skill && skill->isVisible()) {
    ...
    foreach (QString skill_name, skill_names) {
        if (skill_name.startsWith("-")) {
            ...
            if (skill && skill->isVisible()) {
                ...
                QList<const Skill *> relatedSkills = Sanguosha->getRelatedSkills(actual_skill);
                foreach (const Skill *skill, relatedSkills) {
                    if (NULL != skill && !skill->isVisible()) {
                        detachSkillFromPlayer(player, skill->objectName());
                    }
...
ServerPlayer *Room::getRaceResult(QList<ServerPlayer *> &players, QSanProtocol::CommandType, time_t timeOut,
    ...
    foreach (ServerPlayer *player, players) {
        ...
        player->m_expectedReplySerial = static_cast<unsigned int>(-1);  //m_expectedReplySerial = UINT_MAX
        player->releaseLock(ServerPlayer::SEMA_MUTEX);
    }
    _m_semRoomMutex.release();
...
bool Room::askForSkillInvoke(ServerPlayer *player, const QString &skill_name, const QVariant &data) {
    ...
    if (ai) {
        ...
        if (data.type() == QVariant::String)
            ...
            //由于卡牌在灰化时也能弹出右键菜单了，为了在武器重铸时能继续保持
            //“可以右键双击表示不发动该技能而是装备武器”的功能，需要对武器牌
            //在重铸时做特殊处理，阻止在该武器牌上右键单击时弹出菜单
            if (skill_name == "weapon_recast") {
                CardUseStruct cardUse = data.value<CardUseStruct>();
                skillCommand = toJsonArray(skill_name, data_str,
                    QString::number(cardUse.card->getId()));
            }
            else {
                skillCommand = toJsonArray(skill_name, data_str);
            }
...
const Card *Room::askForCard(ServerPlayer *player, const QString &pattern, const QString &prompt,
    ...
    if (card) {
        if (!card->isVirtualCard()) {
            ...
            if (NULL != wrapped && wrapped->isModified())
...
const Card *Room::askForSinglePeach(ServerPlayer *player, ServerPlayer *dying) {
    ...
    if (ai) {
        ...
        //对拥有"完杀"技能的角色的当前回合作特殊处理
        ServerPlayer *jiaxu = getCurrent();
        if (jiaxu && jiaxu->hasSkill("wansha")
            && jiaxu->getPhase() != Player::NotActive
            && player != dying) {
                delay = 0;
        }
        else {
            delay = (Config.AIDelay / 2);
        }
...
ServerPlayer *Room::addSocket(ClientSocket *socket) {
    ...
    connect(player, SIGNAL(request_got(const QString &)), this, SLOT(processClientPacket(const QString &)));
...
void Room::processClientPacket(const QString &request) {
    ...
    if (packet.parse(request.toStdString())) {
        ServerPlayer *player = qobject_cast<ServerPlayer *>(sender());
        if (game_finished) {
...
bool Room::addRobotCommand(ServerPlayer *player, const Json::Value &) {
    ...
    QStringList allocatedRobotNames;
    int n = 0;
    foreach (ServerPlayer *player, m_players) {
        if (player->getState() == "robot") {
            ++n;
            allocatedRobotNames << player->screenName();
        }
    }
    ...
    if (Config.HostAddress == "127.0.0.1") {
        int i = 0;
        int j = m_players.size();
        for (; i < j; ++i) {
            robot_name = Sanguosha->getRandomRobotName();
            if (allocatedRobotNames.contains(robot_name)) {
                continue;
            }
            else {
                break;
    ...
    QString greeting = Settings::toBase64(tr("Hello, I'm a robot"));
...
bool Room::toggleReadyCommand(ServerPlayer *player, const Json::Value &) {
    if (game_started)
        return true;
...
void Room::assignGeneralsForPlayers(const QList<ServerPlayer *> &to_assign) {
    ...
    foreach (ServerPlayer *player, m_players) {
        if (player->getGeneral()) {
            ...
            foreach (QString sp_name, Sanguosha->getSpConvertPairs(player->getGeneralName()))
                existed << sp_name;
        }
        if (player->getGeneral2()) {
            existed << player->getGeneral2Name();
            foreach (QString sp_name, Sanguosha->getSpConvertPairs(player->getGeneral2Name()))
                existed << sp_name;
        }
    }
    ...
    const int remainGeneralCount = total - existed.size();
    const int max_available = remainGeneralCount / to_assign.length();
    const int choice_count = qMin(max_choice, max_available);

    if (choice_count <= 0) return;

    QStringList choices = Sanguosha->getRandomGenerals(remainGeneralCount, existed);
    ...
    if (Config.EnableHegemony) {
        if (to_assign.first()->getGeneral()) {
            foreach (ServerPlayer *sp, m_players) {
                ...
                //keep legal generals
                {
                    QStringList tmpList = old_list;
                    foreach (const QString &name, old_list) {
                        if ((NULL != sp->getGeneral()
                            && Sanguosha->getGeneral(name)->getKingdom() != sp->getGeneral()->getKingdom())
                            || sp->findReasonable(tmpList, true) == name) {
                                sp->addToSelected(name);
                                tmpList.removeOne(name);
                        }
                    }

                    old_list.swap(tmpList);
                }
...
void Room::assignGeneralsForPlayersOfJianGeDefenseMode(const QList<ServerPlayer *> &to_assign)
{
    ...
    if (Config.Enable2ndGeneral) {
        foreach (const QString &name, BanPair::getAllBanSet()) {
            const General *gen = Sanguosha->getGeneral(name);
            if (gen)
                existed[gen->getKingdom()] << name;
        }
        if (to_assign.first()->getGeneral()) {
            foreach (const QString &name, BanPair::getSecondBanSet()) {
                const General *gen = Sanguosha->getGeneral(name);
                if (gen)
                    existed[gen->getKingdom()] << name;
            }
    ...
    foreach (const QString &key, Config.JianGeDefenseKingdoms.keys()) {
...
void Room::chooseGenerals(const QList<ServerPlayer *> &playerList) {
    const QList<ServerPlayer *> &players
        = playerList.isEmpty() ? m_players : playerList;
    // for lord.
    ...
    if (!Config.EnableHegemony && the_lord && players.contains(the_lord)) {
        ...
        if (general != NULL) {
            QStringList generals = Sanguosha->getSpConvertPairs(general);
            if (!generals.isEmpty()) {
                generals << general;
                general = askForGeneral(the_lord, generals);
            }
        }
        the_lord->setGeneralName(general);
        the_lord->addToSelected(general);

        if (!Config.EnableBasara) {
            broadcastProperty(the_lord, "general", general);
        }
        else {
            notifyProperty(the_lord, the_lord, "general", general);
        }
    ...
    foreach (ServerPlayer *player, to_assign) {
        ...
        QString generalName = toQString(player->getClientReply());
        if (generalName != NULL) {
            QStringList generals = Sanguosha->getSpConvertPairs(generalName);
            if (!generals.isEmpty()) {
                generals << generalName;
                generalName = askForGeneral(player, generals);
            }
        }
    ...
    if (Config.Enable2ndGeneral) {
        ...
        if (!Config.EnableHegemony) {
            ...
            if (NULL == the_lord->getGeneral2()) {
                QString generalName = toQString(the_lord->getClientReply());
                if (generalName != NULL) {
                    QStringList generals = Sanguosha->getSpConvertPairs(generalName);
                    if (!generals.isEmpty()) {
                        generals << generalName;
                        generalName = askForGeneral(the_lord, generals);
                    }
                }
                if (!the_lord->m_isClientResponseReady || !_setPlayerGeneral(the_lord, generalName, false)) {
                        _setPlayerGeneral(the_lord, _chooseDefaultGeneral(the_lord), false);
                }

                if (!Config.EnableBasara) {
                    broadcastProperty(the_lord, "general2", the_lord->getGeneral2Name());
                }
            }
...
void Room::run() {
    ...
    if (scenario && !scenario->generalSelection())
        ...
        connect(thread_3v3, SIGNAL(finished()), this, SLOT(deleteThread3v3()));
    } else if (mode == "06_XMode") {
        thread_xmode = new RoomThreadXMode(this);
        thread_xmode->start();

        connect(thread_xmode, SIGNAL(finished()), this, SLOT(startGame()));
        connect(thread_xmode, SIGNAL(finished()), this, SLOT(deleteThreadXMode()));
    } else if (mode == "02_1v1") {
        thread_1v1 = new RoomThread1v1(this);
        thread_1v1->start();

        connect(thread_1v1, SIGNAL(finished()), this, SLOT(startGame()));
        connect(thread_1v1, SIGNAL(finished()), this, SLOT(deleteThread1v1()));
    } else if (mode == "04_1v3") {
        ServerPlayer *lord = m_players.first();
        setPlayerProperty(lord, "general", "shenlvbu1");

        QStringList names;
        QStringList genNames = GetConfigFromLuaState(Sanguosha->getLuaState(), "hulao_generals").toStringList();
        foreach (const QString &gen_name, genNames) {
...
QString Room::_chooseDefaultGeneral(ServerPlayer *player) const{
    ...
    if (Config.EnableHegemony && Config.Enable2ndGeneral) {
        QStringList selectedNames = player->getSelected();
        foreach (const QString &name, selectedNames) {
            ...
            if (player->getGeneral() != NULL) { // choosing first general
                ...
                foreach (const QString &other, selectedNames) { // choosing second general
...
#define _NO_BROADCAST_SPEAKING {\
    ...
    if (player && Config.EnableCheat) {
        QString sentence = Settings::fromBase64(toQString(arg.asString()));
        if (sentence == ".BroadcastRoles") {
            ...
            foreach (ServerPlayer *zuoci, zuocis) {
                ...
                huashen = Settings::toBase64(huashen);
...
void Room::startGame() {
    ...
    if (Config.EnableHegemony && Config.Enable2ndGeneral) {
        foreach (ServerPlayer *player, m_players) {
            ...
            const General *primaryGeneral = Sanguosha->getGeneral(generals.first());
            if (NULL != primaryGeneral) {
                if (primaryGeneral->isCompanionWith(Sanguosha->getGeneral(generals.last()))) {
    ...
    //国战模式，在亮将前，玩家是没有任何技能的(general->getSkillList()恒返回空列表)
    //亮将后，技能的添加与性别的设置，在Room::changePlayerGeneral和Room::changePlayerGeneral2中做了处理
...
void Room::updateCardsOnLose(const CardsMoveStruct &move) {
    for (int i = 0; i < move.card_ids.size(); ++i) {
        ...
        if (card->isModified()) {
            if (move.to_place == Player::DiscardPile || move.to_place == Player::PlaceSpecial) {
...
void Room::notifySkillInvoked(const ServerPlayer *player, const QString &skill_name) {
    Json::Value args;
...
void Room::broadcastSkillInvoke(const QString &skill_name, const QString &category, int type) {
    ...
    QString player_name;
    ServerPlayer *player = findPlayerBySkillName(skill_name);
    if (NULL != player) {
        player_name = player->objectName();
    }
    args[4] = toJsonString(player_name);

    doBroadcastNotify(QSanProtocol::S_COMMAND_LOG_EVENT, args);
}

void Room::broadcastSkillInvoke(const QString &skill_name) {
    broadcastSkillInvoke(skill_name, true, -1);
}

void Room::broadcastSkillInvoke(const QString &skill_name, int type) {
    broadcastSkillInvoke(skill_name, true, type);
}
...
void Room::doSuperLightbox(const QString &heroName, const QString &skillName)
{
    ...
    doLightbox("image=" + heroName, 3000);
// 	doAnimate(S_ANIMATE_LIGHTBOX, "image=" + heroName, QString("%1:0").arg(4500));
// 	thread->delay(4500);
}
    ...
    const QList<ServerPlayer *> &players)
{
    ...
    players.isEmpty() ? doBroadcastNotify(m_players, S_COMMAND_ANIMATE, arg)
...
void Room::activate(ServerPlayer *player, CardUseStruct &card_use) {
    ...
    if (ai) {
        ...
        if (!timer.hasExpired(Config.AIDelay)) {
    ...
    if (ai)
        ...
        if (success && clientReply.isString()) {
            ...
            if (!Sanguosha->getKingdoms().contains(result))
                result = "wei";
            //修复“张角或孙策为主公，神将选国籍为'群'或'吴'时没有黄天和制霸技能”的问题
            else {
                if (player->tag.contains("god_huangtianv")) {
                    player->tag.remove("god_huangtianv");

                    if (result == "qun") {
                        attachSkillToPlayer(player, "huangtianv");
                    }
                }
                ...
                if (player->tag.contains("god_zhiba_pindian")) {
                    ...
                    if (result == "wu") {
                        attachSkillToPlayer(player, "zhiba_pindian");
                    }
                }
            }
        }
...
void Room::askForGuanxing(ServerPlayer *zhuge, const QList<int> &cards, GuanxingType guanxing_type) {
    ...
    if (ai) {
        ...
        guanxingArgs[1] = (int)guanxing_type;
...
QList<const Card *> Room::askForPindianRace(ServerPlayer *from, ServerPlayer *to, const QString &reason) {
    if (!from || !from->isAlive() || !to || !to->isAlive())
...
void Room::_setupChooseGeneralRequestArgs(ServerPlayer *player) {
    ...
    if (!Config.EnableBasara) {
        ...
        if (lord) {
            //修复“双将时，看不见主公的第二个武将选择的是谁”的问题
            QString lordGeneralName = lord->getGeneral2Name();
            if (lordGeneralName.isEmpty()) {
                lordGeneralName = lord->getGeneralName();
            }
            options.append(toJsonString(QString("%1(lord)").arg(lordGeneralName)));
        }
    }
    else {
...
void Room::sendLog(const LogMessage &log, const QList<ServerPlayer *> &players) {
    if (log.type.isEmpty())
        return;
    if (players.isEmpty())
    ...
    bool notify_skill)
{
    ...
    if (notify_skill) {
        ...
        const Skill *skill = Sanguosha->getSkill(skill_name);
        if (skill)
            guojia->broadcastSkillInvoke(skill_name, skill->getEffectIndex(target, dummy_card));
...
void Room::delayForArmorJudge()
{
    long delayMSecs = -1;
    if (getTag("ArmorJudge").toString() == "eight_diagram") {
        removeTag("ArmorJudge");
        delayMSecs = 250;
    }
    thread->delay(delayMSecs);
}

void Room::removeArmorJudgeTag()
{
    if (getTag("ArmorJudge").toString() == "eight_diagram") {
        removeTag("ArmorJudge");
    }
...
void Room::blockAllThreadSignals(bool block)
{
    if (thread) thread->blockSignals(block);
    if (thread_1v1) thread_1v1->blockSignals(block);
    if (thread_3v3) thread_3v3->blockSignals(block);
    if (thread_xmode) thread_xmode->blockSignals(block);
...
void Room::setAnjiangNames(ServerPlayer *player, const QStringList &anJiangNames)
{
    if (player->getState() != "robot" && !anJiangNames.isEmpty()) {
        Json::Value args;
        args[0] = QSanProtocol::S_GAME_EVENT_SET_ANJIANG_NAMES;
        args[1] = toJsonString(player->objectName());
        args[2] = toJsonArray(anJiangNames);
        doNotify(player, S_COMMAND_LOG_EVENT, args);
    }
}

void Room::removeAnjiangNames(ServerPlayer *player)
{
    if (player->getState() != "robot") {
        doNotify(player, S_COMMAND_REMOVE_ANJIANG_NAMES, toJsonString(player->objectName()));
    }
}

void Room::deleteThread1v1()
{
    if (NULL != thread_1v1) {
        delete thread_1v1;
        thread_1v1 = NULL;
...
void Room::deleteThreadXMode()
{
    if (NULL != thread_xmode) {
        delete thread_xmode;
        thread_xmode = NULL;
    }
...
void Room::broadcastPhaseEffect(TriggerEvent triggerEvent, const QVariant &data)
{
    Json::Value args;
    args[0] = QSanProtocol::S_GAME_EVENT_PLAY_PHASE_EFFECT;
    args[1] = triggerEvent;
    if (QVariant::String == data.type()) {
        args[2] = toJsonString(data.toString());
    }

    doBroadcastNotify(QSanProtocol::S_COMMAND_LOG_EVENT, args);
}

void Room::disconnectPlayer(ServerPlayer *player)
{
    ...
    if (player->objectName().isEmpty()) {
    ...
    } else if (player->getRole().isEmpty() && m_players.length() < player_count) {
        // second case
        m_players.removeOne(player);
        ...
        if (!someone_is_online) {
            if (!game_finished) {
                ...
                if (NULL != thread) {
                    thread->m_mutexForRequestExit.lock();
                    thread->m_requestExit = true;
                    thread->m_mutexForRequestExit.unlock();
                }
            }
...
ServerPlayer *Room::findPlayerByName(const QString &name) const
{
    foreach (ServerPlayer *player, m_players) {
...
bool Room::kickPlayerCommand(ServerPlayer *, const Json::Value &arg)
{
    QString kickedPlayerName = toQString(arg);
    ServerPlayer *player = findPlayerByName(kickedPlayerName.trimmed());
    if (player && (player->getState() == "online" || player->getState() == "trust")) {
        player->setKicked(true);
        notifyProperty(player, player, "kicked");
...
void Room::updateSkills()
{
    Json::Value args;
    args[0] = QSanProtocol::S_GAME_EVENT_REFASHION_SKILL;
    doBroadcastNotify(QSanProtocol::S_COMMAND_LOG_EVENT, args);
}
