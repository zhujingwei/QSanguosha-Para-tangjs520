...
public:
    ...
    Room(QObject *parent, const QString &mode);
    ~Room();
    ...
    //在toggleFullSkin时，需要暂时中断一下信号，以免异常
    void blockAllThreadSignals(bool block);

    void setAnjiangNames(ServerPlayer *player, const QStringList &anJiangNames);
    void removeAnjiangNames(ServerPlayer *player);
    ...
    const QList<ServerPlayer *> &getPlayers() const { return m_players; }
    QList<ServerPlayer *> getAllPlayers(bool include_dead = false) const;
    const QList<ServerPlayer *> &getAlivePlayers() const { return m_alivePlayers; }
    ...
    inline void doAnimate(int type, const QString &arg1 = QString(),
        const QString &arg2 = QString(),
        const QList<ServerPlayer *> &players = QList<ServerPlayer *>()) {
    ...
    void adjustSeats();
    void swapPile();
    QList<int> getDiscardPile() { return *m_discardPile; }
    inline QList<int> &getDrawPile() { return *m_drawPile; }
    int getCardFromPile(const QString &card_name);

    ServerPlayer *findPlayerByName(const QString &name) const;
...
private:
    ...
    bool gameStartCommandNotified;
    bool startInXsCommandNotified;
    ...
    void delayForArmorJudge();
    void removeArmorJudgeTag();

    static bool isSkillValidForPlayer(const ServerPlayer *player, const Skill *skill);
    void disconnectPlayer(ServerPlayer *player);
...
private slots:
    ...
    void deleteThread1v1();
    void deleteThread3v3();
    void deleteThreadXMode();
    void deleteThread();
