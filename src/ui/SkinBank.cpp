...
const char *QSanRoomSkin::S_SKIN_KEY_OWNER_ICON = "playerOwner";
...
const char *QSanRoomSkin::S_HERO_SKIN_KEY_PLAYER_AUDIO_EFFECT = "playerAudioEffect-%1-%2_%3-%4";
...
SkillToAudioFiles QSanRoomSkin::m_HeroSkinSkillAudioBank;
QMutex MutexLocker::m_mutex;
QHash<QString, SkillToAudioFiles> QSanRoomSkin::m_GeneralToSkillToAudioFiles;
...
QPixmap QSanRoomSkin::getButtonPixmap(const QString &groupName,
    ...
    if (path.isNull()) return getNullPixmap();
...
QPixmap QSanRoomSkin::getProgressBarPixmap(int percentile) const{
    ...
    if (!allMaps.isArray()) return getNullPixmap();
    ...
    for (unsigned int i = 0; i < allMaps.size(); ++i) {
        ...
        if (thred >= percentile) {
            ...
            return getPixmapFromFileName(toQString(allMaps[i][1]), true);
        }
    }
...
QString QSanRoomSkin::getPlayerAudioEffectPath(const QString &eventName, const QString &category, int index) const{
    ...
    QString fileName = getAudioFileNameFromKey(key, index);
    if (fileName.isEmpty()) {
        const Skill *skill = Sanguosha->getSkill(eventName);
        QStringList fileNames;
        if (skill) fileNames = skill->getSources();
		QString file_path = QString("general/%1/%2").arg(category).arg(eventName);
...
QString QSanRoomSkin::getHeroSkinAudioEffectPath(const QString &generalName, int generalSkinIndex,
    const QString &skillName, const QString &category, int index) const
{
    QString key = QString(QSanRoomSkin::S_HERO_SKIN_KEY_PLAYER_AUDIO_EFFECT).arg(category)
        .arg(generalName).arg(generalSkinIndex).arg(skillName);

    QString fileName = getAudioFileNameFromKey(key, index);
    if (fileName.isEmpty()) {
        fileName = getHeroSkinAudioFileNameFromSkill(generalName, skillName, index);
    }

    return fileName;
}

QString QSanRoomSkin::getAudioFileNameFromKey(const QString &key, int index) const
{
    QString fileName;
    if (index < 0) {
...
QString QSanRoomSkin::getHeroSkinAudioFileNameFromSkill(const QString &generalName,
    const QString &skillName, int index) const
{
    QString fileName;
    QStringList fileNames = getHeroSkinSKillAudios(generalName, skillName);
    if (!fileNames.isEmpty()) {
        if (index < 0)
            fileName = fileNames.at(qrand() % fileNames.length());
        else {
...
#include <QDir>
void QSanRoomSkin::addHeroSkinSkillAudioToBank(const QString &key,
    const QString &generalFullName, const QString &skillName)
{
    QStringList audios;
    QString basePath = QString("image/heroskin/audio/%1/skill").arg(generalFullName);

    QDir dir(basePath);
    dir.setNameFilters(QStringList(QString("%1*.ogg").arg(skillName)));
    dir.setSorting(QDir::Name | QDir::IgnoreCase);
    QStringList files = dir.entryList(QDir::Files | QDir::NoDotAndDotDot);
    foreach (const QString &file, files) {
        QString effect_file = QString("%1/%2").arg(basePath).arg(file);
        audios << effect_file;
    }

    m_HeroSkinSkillAudioBank.insert(key, audios);
}

QString QSanRoomSkin::getMatchGeneralNameFromAudioConfig(const QString &generalName,
    int generalSkinIndex, const QString &category) const
{
    return toQString(
        _m_audioConfig[QString(S_HERO_SKIN_KEY_PLAYER_AUDIO_EFFECT)
        .arg(category).arg(generalName).arg(generalSkinIndex).arg("default").toAscii().constData()]);
}

QStringList QSanRoomSkin::getHeroSkinSKillAudios(const QString &generalName,
    const QString &skillName) const
{
    ...
    if (0 != generalSkinIndex) {
        QString generalFullName = QString("%1_%2").arg(generalName).arg(generalSkinIndex);
        result = _getHeroSkinSKillAudiosHelper(generalFullName, skillName);
        if (result.isEmpty()) {
            const General *general = Sanguosha->getGeneral(generalName);
            if (NULL != general) {
                QString matchGeneralFullName = getMatchGeneralNameFromAudioConfig(
                    generalName, generalSkinIndex, general->getGenderString());
                if (!matchGeneralFullName.isEmpty()) {
                    result = _getHeroSkinSKillAudiosHelper(matchGeneralFullName, skillName);
                }
            }
        }
    }
...
QStringList QSanRoomSkin::_getHeroSkinSKillAudiosHelper(const QString &generalFullName,
    const QString &skillName) const
{
    QString key = QString("%1-%2").arg(generalFullName).arg(skillName);

    MutexLocker mutexLocker;
    if (!m_HeroSkinSkillAudioBank.contains(key)) {
        addHeroSkinSkillAudioToBank(key, generalFullName, skillName);
    }

    return m_HeroSkinSkillAudioBank.value(key);
}

SkillToAudioFiles QSanRoomSkin::getSkillToAudioFiles(const QString &generalFullName)
{
    if (!m_GeneralToSkillToAudioFiles.contains(generalFullName)) {
        SkillToAudioFiles skillToAudioFiles;
        QStringList audios;
        QString basePath = QString("image/heroskin/audio/%1/skill").arg(generalFullName);
        ...
        dir.setSorting(QDir::Name | QDir::IgnoreCase);
        QStringList files = dir.entryList(QDir::Files | QDir::NoDotAndDotDot);
        QRegExp rx("([A-Za-z_]+)(?:\\d?).ogg");
        QString current_skill_name;
        foreach (const QString &file, files) {
            if (rx.exactMatch(file)) {
                QString skill_name = rx.capturedTexts().at(1);
                if (current_skill_name.isEmpty()) {
                    current_skill_name = skill_name;
                }
                else if (current_skill_name != skill_name) {
                    skillToAudioFiles.insert(current_skill_name, audios);
                    current_skill_name = skill_name;
                    audios.clear();
                }
        ...
        m_GeneralToSkillToAudioFiles.insert(generalFullName, skillToAudioFiles);
    }

    return m_GeneralToSkillToAudioFiles.value(generalFullName);
}
...
QPixmap IQSanComponentSkin::getPixmap(const QString &key, const QString &arg, bool cache) const{
    ...
    if (isImageKeyDefined(totalKey)) { // read from config file
        ...
        if (!QFile::exists(fileName)) {
            bool tryDefaultKey = true;
            if (key.contains("playerGeneralIcon")) {
...
void IQSanComponentSkin::clear()
{
    _m_imageConfig.clear();
    _m_audioConfig.clear();
    _m_animationConfig.clear();
}

#include "GenericCardContainerUI.h"

QAbstractAnimation *QSanRoomSkin::createHuaShenAnimation(QPixmap &huashenAvatar,
    QPoint topLeft, QGraphicsItem *parent,
    GraphicsWidgetHoverItem *&huashenAvatarCreated) const
{
    QLabel *avatar = new QLabel;
    avatar->setStyleSheet("QLabel { background-color: transparent; }");
    avatar->setPixmap(huashenAvatar);

    GraphicsWidgetHoverItem *widget = new GraphicsWidgetHoverItem(parent);
...
SanShadowTextFont QSanRoomSkin::DashboardLayout::getSkillTextFont(QSanButton::ButtonState state,
    QSanInvokeSkillButton::SkillType type,
    QSanInvokeSkillButton::SkillButtonWidth width) const
{
    int i = QSanButton::S_NUM_BUTTON_STATES * (int)type + (int)state;
    SanShadowTextFont font = m_skillTextFonts[width];
    font.setColor(m_skillTextColors[i]);
    font.setShadowColor(m_skillTextShadowColors[i]);
    return font;
}
...
bool QSanRoomSkin::_loadLayoutConfig(const Json::Value &layoutConfig) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        if (!layout->m_equipBlackPointFont.tryParse(playerConfig["equipBlackPointFont"])) {
            layout->m_equipBlackPointFont.tryParse(playerConfig["equipPointFont"]);
        }
        if (!layout->m_equipRedPointFont.tryParse(playerConfig["equipRedPointFont"])) {
            layout->m_equipRedPointFont = layout->m_equipBlackPointFont;
            layout->m_equipRedPointFont.setColor(QColor(153, 27, 30));
        }
...
const QPixmap &QSanRoomSkin::getNullPixmap()
{
    static QPixmap nullPixmap;
    return nullPixmap;
}

void QSanRoomSkin::getHeroSkinContainerGeneralIconPathAndClipRegion(const QString &generalName,
    int skinIndex, QString &generalIconPath, QRect &clipRegion) const
{
    if (_m_imageConfig.isObject()) {
        ...
        for (i = 0; i < key_count; ++i) {
            skin_key = keys[i].toStdString();
            if (_m_imageConfig.isMember(skin_key)) {
                break;
            }
        }
        ...
        if (i < key_count) {
            QString path;
            Json::Value val = _m_imageConfig[skin_key];
            if (val.isArray() && val.size() >= 2
                && val[0].isString() && tryParse(val[1], clipRegion)) {
                    path = val[0].asCString();
            }
...
QString QSanSkinFactory::_m_skinListFileName = "skins/skinList.json";

QSanSkinFactory &QSanSkinFactory::getInstance() {
    if (_sm_singleton == NULL)
        _sm_singleton = new QSanSkinFactory();
...
bool QSanSkinFactory::switchSkin(const QString &skinName) {
    //由于增加了支持实时切换全幅与普通界面的功能，
    //所以每次切换skin时都得重新给S_DEFAULT_SKIN_NAME和S_COMPACT_SKIN_NAME赋值
    ...
    _sm_currentSkin.clear();

    bool success = _sm_currentSkin.load(_m_skinList[S_DEFAULT_SKIN_NAME.toAscii().constData()]);
...
QSanSkinFactory::QSanSkinFactory() {
    ...
    QFile file(_m_skinListFileName);
...
GraphicsWidgetHoverItem::GraphicsWidgetHoverItem(QGraphicsItem *parent)
    : QGraphicsProxyWidget(parent)
{
    setAcceptHoverEvents(true);
}

void GraphicsWidgetHoverItem::hoverEnterEvent(QGraphicsSceneHoverEvent *)
{
    emit hover_enter();
}

void GraphicsWidgetHoverItem::hoverLeaveEvent(QGraphicsSceneHoverEvent *)
{
    emit hover_leave();
}
