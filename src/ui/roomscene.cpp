...
RolesBoxItem::RolesBoxItem()
{
    QString backgroundImagePath = "image/system/state.png";
    if (QFile::exists(backgroundImagePath)) {
        QString objName = "stateBox";
        setObjectName(objName);

        setStyleSheet(
            QString("QFrame#%1{border-image: url(%2);background-color: transparent;}")
            .arg(objName)
            .arg(backgroundImagePath));
    }
    else {
        //使用sanguosha.qss文件中QFrame#RolesBox的styleSheet定义
        setObjectName("RolesBox");
    }
}
    ...
    pindian_success(false), _m_currentStage(0)
{
    ...
    m_notShowTargetsEnablityAnimation = false;
    m_choiceDialog = NULL;
    ...
    for (int i = 0; i < player_count - 1; ++i) {
        ...
        connect(photo, SIGNAL(card_to_use()), this, SLOT(doOkButton()));
    }
    ...
    connect(general_changer, SIGNAL(general_chosen(const QString &)),
        this, SLOT(changeGeneral(const QString &)));
    ...
    connect(ClientInstance, SIGNAL(guanxing(QList<int>, int)),
        guanxing_box, SLOT(doGuanxing(QList<int>, int)));
    ...
    connect(ClientInstance, SIGNAL(skill_all_detached()), this, SLOT(detachAllSkills()));
    ...
    chat_edit_widget->setParent(this);
    ...
    //设置垂直滚动条的名称，以便允许通过调用setStyleSheet函数或提供qss文件来修改其样式
    QScrollBar *chatBoxVScrollBar = chat_box->verticalScrollBar();
    QScrollBar *logBoxVScrollBar = log_box->verticalScrollBar();

    if (NULL != chatBoxVScrollBar) {
        chatBoxVScrollBar->setObjectName("sgsVSB");
        chatBoxVScrollBar->setStyleSheet(Settings::getQSSFileContent());
    }
    ...
    m_timerLabel->setPos(QPointF(-1, -1));
    m_timerLabel->resize(52, 18);
    ...
    m_rolesBoxItem = new RolesBoxItem();
    m_rolesBox = addWidget(m_rolesBoxItem);
    m_rolesBox->setParent(this);
	...
	if (!ClientInstance->isReplayState() && ServerInfo.EnableAI) {
		...
		add_robot->setTransform(QTransform::fromTranslate(-add_robot->boundingRect().width() / 2, -add_robot->boundingRect().height() / 2));
		...
		fill_robots->setTransform(QTransform::fromTranslate(-fill_robots->boundingRect().width() / 2, -fill_robots->boundingRect().height() / 2));
		add_robot->setPos(0, add_robot->boundingRect().height() + 10);
...
void RoomScene::handleGameEvent(const Json::Value &arg) {
    ...
    case S_GAME_EVENT_PLAY_EFFECT: {
            ...
            //兼容老版本的服务器(只有3个参数)
            if (arg.isValidIndex(4)) {
                QString playerName = arg[4].asCString();
            ...
            //支持武将皮肤与配音关联
            if (!playHeroSkinAudioEffect(player, skillName, category, type)) {
                QString audioPath = G_ROOM_SKIN.getPlayerAudioEffectPath(skillName, category, type);
                bool continuePlayWhenPlaying = true;
                if (audioPath.startsWith("audio/skill", Qt::CaseInsensitive)) {
                    continuePlayWhenPlaying = false;
                }
                Sanguosha->playAudioEffect(audioPath, continuePlayWhenPlaying);
            }
    ...
    case S_GAME_EVENT_SET_ANJIANG_NAMES: {
        ClientPlayer *player = ClientInstance->getPlayer(arg[1].asCString());
        QStringList anJiangNames;
    ...
    case S_GAME_EVENT_PLAY_PHASE_EFFECT: {
        TriggerEvent eventType = static_cast<TriggerEvent>(arg[1].asInt());
        if (GameStart == eventType) {
            Sanguosha->playSystemAudioEffect("game-start");
        }
        break;
    }
    ...
    case S_GAME_EVENT_REFASHION_SKILL: {
        refashionSkills();
    }
    default:
            break;
...
QGraphicsPixmapItem *RoomScene::createDashboardButtons() {
    QGraphicsPixmapItem *widget = new QGraphicsPixmapItem(G_ROOM_SKIN.getPixmap
...
void RoomScene::adjustItems() {
    ...
    if (skinName == factory.S_DEFAULT_SKIN_NAME) {
        if (displayRegion.width() < minSize.width() || displayRegion.height() < minSize.height()) {
            ...
            Sanguosha->blockAllRoomSignals(false);
            updatePhotosAndDashboard();
        }
    ...
    m_rolesBox->resize(_m_infoPlane.width(), _m_roomLayout->m_roleBoxHeight);
...
void RoomScene::updateTable() {
    ...
    m_tableRect = QRectF(col1, row1, col2 - col1, row2 - row1);

    QRect tableBottomBar(0, 0, log_box_widget->x() - col1, G_DASHBOARD_LAYOUT.m_floatingAreaHeight);
    tableBottomBar.moveBottomLeft(QPoint((int)m_tableRect.left(), 0));
    ...
    m_tablePile->setPos(m_tableCenterPos.x(), m_tableCenterPos.y() - 10);
    m_tablePile->setSize(qMax((int)m_tableRect.width() - _m_roomLayout->m_discardPilePadding * 2,
                         _m_roomLayout->m_discardPileMinWidth), _m_commonLayout->m_cardNormalHeight);
    m_tablePile->adjustCards();

    int cardContainerPosY = Sanguosha->getPlayerCount(ServerInfo.GameMode) > 8
        ? m_tableCenterPos.y() - 58 : m_tableCenterPos.y() - 75;
    card_container->setPos(m_tableCenterPos.x() + 12, cardContainerPosY);

    guanxing_box->setPos(m_tableCenterPos.x() + 12, m_tableCenterPos.y() - 34);
...
void RoomScene::updateTargetsEnablity(const Card *card) {
    ...
    while (itor.hasNext()) {
        ...
        if (!m_notShowTargetsEnablityAnimation) {
            QGraphicsItem *animationTarget = item->getMouseClickReceiver();
            if (enabled)
                animations->effectOut(animationTarget);
        ...
        //支持显示“可烈弓”提示功能
        bool hasLieGongSkill = Self->hasSkill("liegong");
        bool hasKofLieGongSkill = Self->hasSkill("kofliegong");
        if (hasLieGongSkill || hasKofLieGongSkill) {
            if (Player::Play == Self->getPhase()
                && NULL != card && card->isKindOf("Slash") && !card->hasFlag("card_used")
                && enabled) {
                int handcardnum = player->getHandcardNum();
                if (Self->getHp() <= handcardnum
                    || (Self->getAttackRange() >= handcardnum && hasLieGongSkill)) {
                    item->showSkillPromptInfo(tr("CanLiegong"));
                }
            }
            else {
                item->hideSkillName();
...
void RoomScene::keyReleaseEvent(QKeyEvent *event) {
    ...
    //新增ALT+X在游戏过程中随时退出
    case Qt::Key_X: {
            if (game_started && alt_is_down) {
                MainWindow *mainWnd = qobject_cast<MainWindow *>(main_window);
                if (NULL != mainWnd) {
                    mainWnd->forceRestart();
                }
            }
        }
    }
...
void RoomScene::chooseOption(const QString &skillName, const QStringList &options) {
    ...
    if (skillName.contains("guhuo")) {
        QString specialSkillLogText = log_box->getSpecialSkillLogText();
        if (!specialSkillLogText.isEmpty()) {
            QLabel *specialSkillLogLabel = new QLabel(specialSkillLogText, dialog);
            specialSkillLogLabel->setObjectName("SkillLog");
            layout->addWidget(specialSkillLogLabel);
            log_box->clearSpecialSkillLogText();
        }
    }
...
GenericCardContainer *RoomScene::_getGenericCardContainer(Player::Place place, const Player *player) const {
...
QString RoomScene::_translateMovement(const CardsMoveStruct &move) {
    ...
    if (!reason.m_eventName.isEmpty()) {
        QString eventName = Sanguosha->translate(reason.m_eventName);
        if (eventName != reason.m_eventName) {
            result.append(eventName);
        }
    }
    if (!reason.m_skillName.isEmpty()) {
        QString skillName = Sanguosha->translate(reason.m_skillName);
        if (skillName != reason.m_skillName) {
...
void RoomScene::addSkillButton(const Skill *skill) {
    ...
    // check duplication
    if (dashboard->getSkillButtonByName(skill->objectName()))
        return;
...
//                     Self->setFlags("-" + skill_name);
    ...
    case Client::AskForYiji: {
            ...
            QStringList yiji_info = Sanguosha->getCurrentCardUsePattern().split("=");
...
void RoomScene::doOkButton() {
    ...
    //如果是双击时通过模拟选中进入该函数的，
    //则通过clearDoubleClickedItem来通知Dashboard或Photo，此次模拟动作有效
    PlayerCardContainer *playerItem = qobject_cast<PlayerCardContainer *>(sender());
    if (playerItem && playerItem->getDoubleClickedItem()) {
        playerItem->clearDoubleClickedItem();
    }
...
void RoomScene::changeHp(const QString &who, int delta, DamageStruct::Nature nature, bool losthp) {
    ...
    if (delta < 0) {
		Sanguosha->playSystemAudioEffect("decHp", true);
...
void RoomScene::addRestartButton(QDialog *dialog) {
    ...
    connect(return_button, SIGNAL(clicked()), dialog, SLOT(reject()));
...
void RoomScene::makeDamage() {
    ...
    DamageMakerDialog damage_maker(main_window);
    damage_maker.exec();
}
...
void RoomScene::makeKilling() {
    ...
    DeathNoteDialog dialog(main_window);
    dialog.exec();
}
...
void RoomScene::doScript() {
    ScriptExecutor dialog(main_window);
    dialog.exec();
}
...
void RoomScene::fillTable(QTableWidget *table, const QList<const ClientPlayer *> &players) {
    ...
    RecAnalysis record(qobject_cast<MainWindow *>(main_window)->getReplayPath());
    ...
    for (int i = 0; i < players.length(); ++i) {
        ...
        if (ServerInfo.EnableHegemony) {
            ...
            item->setIcon(icon);
            item->setText(Sanguosha->translate(kingdom));
        } else {
        ...
        QString handcards = Settings::fromBase64(player->property("last_handcards").toString());
...
void RoomScene::takeAmazingGrace(ClientPlayer *taker, int card_id, bool move_cards) {
    ...
    CardItem *copy = NULL;
    QList<CardItem *> copys = card_container->removeCardItems(card_ids, Player::PlaceHand);
    if (!copys.isEmpty()) {
        copy = copys.first();
    }
    ...
    foreach (CardItem *cardItem, copys) {
        delete cardItem;
    }
}
...
void RoomScene::detachSkill(const QString &skill_name)
{
    ...
    if (Self != NULL && Self->getMark("ViewAsSkill_" + skill_name + "Effect") > 0) {
        Self->setMark("ViewAsSkill_" + skill_name + "Lost", 1);
    } else {
...
void RoomScene::detachAllSkills()
{
    dashboard->removeAllSkillButtons();
...
void RoomScene::speak() {
    if (game_started && ServerInfo.DisableChat)
        ...
        if (text == ".StartBgMusic") {
            ...
            Audio::playBackgroundMusic(bgmusic_path, Config.RandomPlayBGM);
#endif
...
#ifdef AUDIO_SUPPORT
            Audio::stopBackgroundMusic();
            Audio::playBackgroundMusic(path, Config.RandomPlayBGM);
#endif
...
void RoomScene::onGameStart() {
    ...
    if (Self->hasFlag("marshalling")) {
        dashboard->setReadyIconVisible(false);
    }
...
void RoomScene::freeze() {
    game_started = false;
    m_timerLabel->stop();
    //游戏结束后，需停止换肤动画，否则在弹出的战况对话框中立即点击“重新开始”按钮后，程序会崩溃
...
void RoomScene::setEmotion(const QString &who, const QString &emotion) {
    if (emotion.startsWith("weapon/") || emotion.startsWith("armor/")) {
        ...
        Sanguosha->playAudioEffect(G_ROOM_SKIN.getPlayerAudioEffectPath(name, QString("equip")), true);
    }
    ...
    if (photo) {
        ...
        if (pma) {
            ...
            //武器动画显示位置往右移一点，以免与"杀"的动画位置重叠
            else if (emotion.startsWith("weapon/")) {
                pma->moveBy(150, -dashboard->boundingRect().height() / 1.5 - 40);
            }
            else {
...
void RoomScene::doMovingAnimation(const QString &name, const QStringList &args) {
    ...
    QPropertyAnimation *move = new QPropertyAnimation(item, "pos", this);
    move->setStartValue(from);
    move->setEndValue(to);
    move->setDuration(700);

    QPropertyAnimation *disappear = new QPropertyAnimation(item, "opacity", this);
    disappear->setEndValue(0.0);
    disappear->setDuration(700);
...
void RoomScene::doAppearingAnimation(const QString &name, const QStringList &args) {
    ...
    if (fromItem == dashboard) {
        ...
        from = rect.topLeft();
        from.rx() += ((rect.width() - item->boundingRect().width()) / 2);
    }
    else {
        from = fromItem->scenePos();
        from.rx() += ((fromItem->boundingRect().width() - item->boundingRect().width()) / 2);
    }
    item->setPos(from);
    item->setZValue(ULONG_MAX);
...
void RoomScene::doLightboxAnimation(const QString &, const QStringList &args)
{
    ...
    if ((!fileName.isEmpty() && QFile::exists(fileName)) || fileName.isEmpty()) {
        ...
        if (!fileName.isEmpty()) {
            if (imageAnimate) {
                QSanSelectableItem *line = new QSanSelectableItem(fileName);
                addItem(line);
                ...
                QPropertyAnimation *appear = new QPropertyAnimation(line, "opacity", this);
                ...
                PixmapAnimation *pma = PixmapAnimation::GetPixmapAnimation(lightbox, fileName);
                if (pma) {
                    pma->setZValue(20002.0);
                    pma->moveBy(-sceneRect().width()
                        * _m_roomLayout->m_infoPlaneWidthPercentage / 2, 0);
...
void RoomScene::doHuashen(const QString &, const QStringList &args) {
    ...
    foreach (const QString &arg, hargs) {
...
void RoomScene::startArrange(const QString &to_arrange) {
    ...
    if (ServerInfo.GameMode == "06_XMode") {
        ...
        foreach (const QString &name, to_arrange.split("+")) {
            CardItem *item = new CardItem(name);
            item->setObjectName(name);
...
void RoomScene::showPindianBox(const QString &from_name, int from_id, const QString &to_name, int to_id, const QString &reason) {
    ...
    pindian_from_card->setPos(QPointF(31 + pindian_from_card->boundingRect().width() / 2,
        47 + pindian_from_card->boundingRect().height() / 2));
    ...
    pindian_to_card->setPos(QPointF(129 + pindian_to_card->boundingRect().width() / 2,
        47 + pindian_to_card->boundingRect().height() / 2));
...
void RoomScene::setChatBoxVisible(bool show) {
    if (!show) {
        ...
        _m_infoPlane.setBottom(dashboard->y() + dashboard->middleFrameAndRightFrameHeightDiff());

        log_box->resize(_m_infoPlane.width(), _m_infoPlane.height());
    } else {
        ...
        _m_infoPlane.setBottom(dashboard->y() - _m_roomLayout->m_chatTextBoxHeight
            + dashboard->middleFrameAndRightFrameHeightDiff());
...
void RoomScene::pause()
{
    ...
    const QList<const ClientPlayer *> &clientPlayers = ClientInstance->getPlayers();
    if (clientPlayers.length() < Sanguosha->getPlayerCount(ServerInfo.GameMode)) {
        return;
    }

    foreach (const ClientPlayer *p, clientPlayers) {
...
void RoomScene::startAssign() {
    RoleAssignDialog dialog(main_window);
    dialog.exec();
}

void RoomScene::updatePhotosAndDashboard()
{
...
void RoomScene::toggleFullSkin()
{
    if (QSanSkinFactory::isEnableFullSkin()) {
        bool useFullSkin = !Config.value("UseFullSkin", false).toBool();
        Config.setValue("UseFullSkin", useFullSkin);

        QSanSkinFactory &factory = QSanSkinFactory::getInstance();
        const QString &skinName = factory.getCurrentSkinName();
        bool defaultSkin = skinName.contains("default");

        Sanguosha->blockAllRoomSignals(true);

        factory.switchSkin(defaultSkin ? factory.S_DEFAULT_SKIN_NAME
            : factory.S_COMPACT_SKIN_NAME);
...
bool RoomScene::playHeroSkinAudioEffect(ClientPlayer *player, const QString &skillName,
    const QString &category, int index)
{
    if (NULL != player) {
        QString generalName1, generalName2;
        int generalSkinIndex1 = 0, generalSkinIndex2 = 0;
        getActualGeneralNameAndSkinIndex(player, skillName,
            generalName1, generalSkinIndex1, generalName2, generalSkinIndex2);

        QString audioEffectPath;
        if (0 != generalSkinIndex1) {
            audioEffectPath = G_ROOM_SKIN.getHeroSkinAudioEffectPath(generalName1,
                generalSkinIndex1, skillName, category, index);
        }
        if (!audioEffectPath.isEmpty()) {
            Sanguosha->playAudioEffect(audioEffectPath);
            return true;
        }

        if (0 != generalSkinIndex2) {
            audioEffectPath = G_ROOM_SKIN.getHeroSkinAudioEffectPath(generalName2,
                generalSkinIndex2, skillName, category, index);
        }
        if (!audioEffectPath.isEmpty()) {
            Sanguosha->playAudioEffect(audioEffectPath);
            return true;
...
void RoomScene::getActualGeneralNameAndSkinIndex(ClientPlayer *player, const QString &skillName,
    QString &generalName1, int &skinIndex1, QString &generalName2, int &skinIndex2)
{
    ...
    skinIndex1 = Config.value(QString("HeroSkin/%1").arg(generalName1), 0).toInt();
    ...
    skinIndex2 = Config.value(QString("HeroSkin/%1").arg(generalName2), 0).toInt();
}
...
void RoomScene::addHeroSkinContainer(ClientPlayer *player,
...
void RoomScene::refashionSkills()
{
    QList<const Skill *> skill_list = Self->getVisibleSkillList();
    QList<QSanInvokeSkillButton *> button_list = dashboard->getAllSkillButtons();
    foreach (QSanSkillButton *btn, button_list) {
