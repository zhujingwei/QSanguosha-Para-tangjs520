...
public:
    ...
    void addToPile(const char *pile_name, int card_id, bool open = true,
                   const QList<ServerPlayer *> &open_players = QList<ServerPlayer *>());
    void addToPile(const char *pile_name, const QList<int> &card_ids, bool open = true,
                   const QList<ServerPlayer *> &open_players = QList<ServerPlayer *>());
    void addToPile(const char *pile_name, const QList<int> &card_ids, bool open,
...
void Room::doScript(const QString &script) {
    ...
    int err = luaL_dostring(L, script.toAscii());
    if (err) {
